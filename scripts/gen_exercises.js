'use strict';
// Genera pool di esercizi matematicamente corretti "per costruzione":
// il valore lo calcola JS stesso, non viene mai scritto a mano, così
// escludiamo errori aritmetici. Uso: node scripts/gen_exercises.js > pool.json
// poi si cura/seleziona a mano il sottoinsieme migliore da mettere in data.js.

function ri(min, max) { return Math.floor(Math.random() * (max - min + 1)) + min; }
function pick(arr) { return arr[ri(0, arr.length - 1)]; }
function riNonZero(min, max) { let n; do { n = ri(min, max); } while (n === 0); return n; }
function fmtNum(n) {
  // arrotonda evitando 0.30000000004 ecc.
  return Math.round(n * 1000) / 1000;
}
function signStr(n) { return n < 0 ? `− ${Math.abs(n)}` : `+ ${n}`; } // solo per display leggibile, non usato nei numeri finali
...
function genEquazioniFacile(n) {
  const out = [];
  for (let i = 0; i < n; i++) {
    const pattern = ri(0, 3);
    let q, x;
    if (pattern === 0) { x = riNonZero(-12, 12); const b = riNonZero(-10, 10); q = `x ${b >= 0 ? '+ ' + b : '− ' + Math.abs(b)} = ${x + b}`; }
    else if (pattern === 1) { x = riNonZero(-12, 12); const b = riNonZero(-10, 10); q = `x ${b >= 0 ? '− ' + b : '+ ' + Math.abs(b)} = ${x - b}`; }
    else if (pattern === 2) { x = riNonZero(-12, 12); const a = riNonZero(2, 9); q = `${a}x = ${a * x}`; }
    else { const a = riNonZero(2, 9); const k = riNonZero(-6, 6); x = a * k; q = `x : ${a} = ${k}`; }
    out.push({ q, answer: String(x), hint: 'Isola la x applicando ad entrambi i membri la stessa operazione inversa.' });
  }
  return out;
}
function genEquazioniMedio(n) {
  const out = [];
  for (let i = 0; i < n; i++) {
    const x = riNonZero(-10, 10);
    const a = riNonZero(2, 8);
    const pattern = ri(0, 2);
    let q;
    if (pattern === 0) { const b = riNonZero(-15, 15); q = `${a}x ${b >= 0 ? '+ ' + b : '− ' + Math.abs(b)} = ${a * x + b}`; }
    else if (pattern === 1) { const c = riNonZero(2, 8); if (c === a) continue; const b = riNonZero(-10, 10); const d = (a - c) * x + b; q = `${a}x ${b >= 0 ? '+ ' + b : '− ' + Math.abs(b)} = ${c}x ${d >= 0 ? '+ ' + d : '− ' + Math.abs(d)}`; }
    else { const m = riNonZero(-8, 8); const rhs = a * (x + m); q = `${a}(x ${m >= 0 ? '+ ' + m : '− ' + Math.abs(m)}) = ${rhs}`; }
    out.push({ q, answer: String(x), hint: 'Porta tutti i termini con la x da un lato e i numeri dall\'altro, cambiando segno quando li sposti.' });
  }
  return out;
}
function genEquazioniDifficile(n) {
  const out = [];
  for (let i = 0; i < n; i++) {
    const x = riNonZero(-8, 8);
    const m = riNonZero(-6, 6), k = riNonZero(1, 5);
    const c = riNonZero(2, 6);
    if (k === c) continue; // altrimenti l'equazione degenera in un'identità/contraddizione
    // k(x+m) + b = c*x + d  -> b scelto libero, d ricavato
    const b = riNonZero(-10, 10);
    const lhs_const = k * m + b;
    const d = k * x + lhs_const - c * x;
    const q = `${k}(x ${m >= 0 ? '+ ' + m : '− ' + Math.abs(m)}) ${b >= 0 ? '+ ' + b : '− ' + Math.abs(b)} = ${c}x ${d >= 0 ? '+ ' + d : '− ' + Math.abs(d)}`;
    out.push({ q, answer: String(x), hint: 'Distribuisci il prodotto a sinistra, poi porta le x da una parte e i numeri dall\'altra.' });
  }
  return out;
}
...
/* ============== Esportazione ============== */
const N = 24; // candidati generati per livello: se ne selezionerà poi una parte a mano

const pools = {
  numeri: { facile: dedupe(genNumeriFacile(N)), medio: dedupe(genNumeriMedio(N)), difficile: dedupe(genNumeriDifficile(N)) },
  potenze: { facile: dedupe(genPotenzeFacile(N)), medio: dedupe(genPotenzeMedio(N)), difficile: dedupe(genPotenzeDifficile(N)) },
  equazioni: { facile: dedupe(genEquazioniFacile(N)), medio: dedupe(genEquazioniMedio(N)), difficile: dedupe(genEquazioniDifficile(N)) },
  geometria: { facile: dedupe(genGeometriaFacile(N)), medio: dedupe(genGeometriaMedio(N)), difficile: dedupe(genGeometriaDifficile(N)) },
  frazioni: { facile: dedupe(genFrazioniFacile(N)), medio: dedupe(genFrazioniMedio(N)), difficile: dedupe(genFrazioniDifficile(N)) },
  algebra: { facile: dedupe(genAlgebraFacile(N)), medio: dedupe(genAlgebraMedio(N)), difficile: dedupe(genAlgebraDifficile(N)) }
};
