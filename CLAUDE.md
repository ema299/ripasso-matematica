...
**`data.js` on disk is always the single source of truth — never a generator script.** `scripts/gen_exercises.js`/`scripts/build_data.js`/`scripts/add_worked_steps.js`/`scripts/build_equazioni_v2.js`/`scripts/finalize_equazioni_v2.js` are one-shot migrations, each run against the `data.js` that existed at the time to produce the `data.js` that exists now — none of them are part of any build pipeline you'd run routinely. Their rerunnability differs and has been verified empirically, not assumed: `build_equazioni_v2.js` locates content by matching old title/question strings that no longer exist after its own migration, so rerunning it throws immediately (the intended, safe failure mode — not a bug). `finalize_equazioni_v2.js`, by contrast, only does lookups/edits that are idempotent by construction (reorder by title/format, overwrite a fixed value, add/strip a field conditioned on its absence/presence) and was confirmed byte-for-byte identical on a second run — safe to rerun, though `data.js` on disk is still the source of truth, not the script. Treat every file in `scripts/` whose header says "one-shot"/"migrazione storica" as a historical record of *how* a past change to `data.js` was derived — if you need to change `data.js` content, either hand-edit it directly (fine for small changes) or write a **new** one-shot Node transform in the same style (see the existing ones for the `vm.runInContext` pattern), never assume an old one applies to your change.

**State persists to two independent `localStorage` keys** (bump the key suffix, e.g. `-v2`, if the shape changes, since there's no migration code): `ripasso-mate-progress-v1` (per-module completion: theory seen / per-level exercises done / quiz score; also, additively, `exerciseCounts: {level: count}` recorded at completion time so a level whose exercise bank later grows — e.g. equazioni after its V2 rewrite — is detected as stale and reopened instead of silently hiding the new content, see `isLevelStale()` in `app.js`) and `ripasso-mate-history-v1` (every attempt ever logged via `logAttempt()`, capped at `HISTORY_MAX`, feeding `computeStats()` for the "Diario di viaggio" screen; entries may additively carry `twin`/`skill` when logged from the equazioni help ladder). The student's name is the `STUDENT_NAME` constant at the top of `app.js` — it's hardcoded, not configurable via UI.
